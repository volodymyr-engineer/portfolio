			...
			{props.anchors.map((anchor, index) => (
				<li key={index} className="w-fit">
					<Link href={`#${anchor.href}`} role="navigation" aria-label={anchor.ariaLabel} className="group flex items-center py-3">
						<span className="nav-indicator mr-4 h-px w-8 bg-slate-400 transition-all group-hover:w-16 group-focus-visible:w-16 motion-reduce:transition-none" />
						<span className="nav-text text-xs font-bold tracking-widest text-slate-400 uppercase">{anchor.label}</span>
					</Link>
				</li>
			))}
		</ul>
